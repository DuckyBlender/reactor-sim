...
impl Plugin for SimulationPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(ReactorState::default())
            ...
            .add_systems(OnEnter(GameState::InGame), (reset_game_over_reason, reset_game_state))
            .add_systems(
                Update,
                (
...
fn interpolate_controls(mut controls: ResMut<ControlSettings>, time: Res<Time>) {
    ...
    // Turbine control is instant (no delay for water flow adjustment)
...
fn simulate_reactor(
    ...
    let effective_target = temperature_target - (flow_rate * (reactor.temperature - ROOM_TEMPERATURE) * 0.3);

    reactor.temperature = smooth_towards(reactor.temperature, effective_target, 0.08 + cooling_effect, delta);
...
fn simulate_turbine(
    ...
    if flow_rate > 0.01 {
        ...
        turbine.temperature = smooth_towards(turbine.temperature, turbine.target_temperature, transfer_speed, delta);
    } else {
